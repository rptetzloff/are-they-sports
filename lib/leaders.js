...
/** Each unbroken spell in charge, in order.
 *
 *  A STINT, not a person. The leaders table is one row per leader and that is
 *  right for a record book -- Harvey Kuenn's 1975 and his 1982-83 are one
 *  career -- but a chart band is a period of time, and one band spanning
 *  1975-1983 would draw straight over Alex Grammas, George Bamberger, Buck
 *  Rodgers and Rene Lachemann, who between them held the job for seven of
 *  those nine years.
 *
 *  The rows arrive already folded: `game_leader.leader` is who HELD the job and
 *  `ran` is who managed the game when that was somebody else, so an ejection or
 *  a three-game illness does not chop a season into stints here. That fold
 *  happens in the load, and this would be unusable without it -- Phil Garner's
 *  2006 Astros are `Cooper(1) Garner(37) Cooper(1)` in the raw data.
 *
 *  Ordered by date, not by the order the query returned. There is no ORDER BY
 *  on the season in `leaderGames`.
 */
export function leaderStints(rows) {
	const byClub = new Map();
	for (const r of rows) {
		if (!r.date) continue;
		if (!byClub.has(r.franchise)) byClub.set(r.franchise, []);
	...
	for (const [franchise, games] of byClub) {
		...
		for (const g of inOrder) {
			...
			cur = {
				leader: g.leader, name: g.name, franchise,
				from: season, to: season,
				firstDate: g.date, lastDate: g.date,
				games: 1,
			};
			stints.push(cur);
		}
	}
	return stints.sort((a, b) => (a.firstDate < b.firstDate ? -1 : a.firstDate > b.firstDate ? 1 : 0));
}
...
/** Which number each leader is: the club's own count of who has held the job.
 *
 *  1..N for the people who held it, and a fractional number for somebody who
 *  only ever stood in -- Mike McCarthy is Green Bay's 14th head coach and Joe
 *  Philbin is 14.1 -- so the official count stays intact and the column still
 *  sorts as a number. That is the football site's rule, kept.
 *
 *  **Per PERSON, where the site numbers per STINT**, and that is a deliberate
 *  difference rather than a shortcut. Ray McLean stood in for Green Bay in 1953
 *  and was the head coach in 1958; the club counts him once, as its fourth. The
 *  site's version keys a span back to the coach's whole career, so a man with
 *  two spans gets two rows carrying identical totals -- McLean appears twice,
 *  both times with his combined record.
 *
 *  So a person who was EVER a regular gets an integer, taken at their first
 *  season; a person who only ever stood in gets `N.k` under the last regular
 *  before them. Somebody who stood in before anyone had held the job at all --
 *  which nothing in this data does, and which a club joining mid-season would --
 *  gets `0.k` rather than no number.
 *
 *  It counts what this repo KNOWS, which for football before 1999 is not the
 *  whole story: Hugh Devore's 1953 spell is an interim the curated file does not
 *  mark, so Green Bay comes out with sixteen regulars where the club says
 *  fifteen. The page already says that interims are only marked from 1999.
 */
export function numberLeaders(leaders) {
	const inOrder = [...leaders].sort((a, b) =>
		(a.firstSeason ?? 0) - (b.firstSeason ?? 0)
		|| (a.lastSeason ?? 0) - (b.lastSeason ?? 0)
	...
	for (const r of inOrder) {
		if (r.interim) {
			stood += 1;
			num.set(r.leader, { num: regular + stood / 10, label: (regular + stood / 10).toFixed(1) });
			continue;
		}
		regular += 1;
		stood = 0;
		num.set(r.leader, { num: regular, label: String(regular) });
	}
	// The input order is kept: this page is sortable and the caller decides.
	return leaders.map((r) => ({ ...r, ...num.get(r.leader) }));
}
...
export function leaderColumns({ ties = false, post = false, titles = false, leaderNoun = 'Leader' } = {}) {
	return [
		// The club's own count. First, because it is what orders the table by
		// default and because a reader looking for "who was the fifteenth" is
		// scanning this column and nothing else.
		column('num', '#', (r) => r.num, { numeric: true, defaultDir: 'asc' }),
