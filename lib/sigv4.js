 ...
 *
 *  An access key and a secret key, which is all a private bucket needs. Signing
 *  adds a header to the request already being made, so the streaming, the gzip
 *  sniffing and the error handling are unchanged and a public URL still goes out
 *  unsigned. Everything here is `node:crypto`.
 *
 *  `@aws-sdk/client-s3` would also do it — measured at 26 packages and 15MB,
 *  after an earlier draft of this comment claimed "about fifty" from nothing.
 *  The reason against it is not size: it fetches through its own client, so the
 *  URL would have to be split into endpoint, bucket and key, path-style
 *  addressing configured, and a second download path maintained beside the
 *  plain-URL one. Sixty lines is the cheaper of the two.
 *
