...
export function computeLeague(clubs, { top = 10, streaksSpanSeasons = true } = {}) {
	...
	const tagged = (pick) => per.flatMap(({ team, r }) =>
		pick(r).map((e) => ({ ...e, club: label(team), teamId: team.id })));
	...
	for (const { team, r } of per) {
		for (const t of r.ties) {
			...
			seenTie.add(key);
			ties.push({ ...t, club: label(team), teamId: team.id });
		}
	}

	// One row per club, which is a league view rather than a merge: how many
	// titles each has won, most first.
	const titles = per
		.map(({ team, r }) => ({
			club: label(team), teamId: team.id,
			won: r.championshipAppearances.filter((a) => a.won).length,
			appearances: r.championshipAppearances.length,
			seasons: r.championshipAppearances.filter((a) => a.won).map((a) => a.season).sort((a, b) => a - b),
		}))
		.filter((c) => c.appearances > 0)
		.sort((a, b) => b.won - a.won || b.appearances - a.appearances || a.club.localeCompare(b.club));
	...
	const allTime = per
		.map(({ team, r }) => {
			...
			const played = w + l + t;
			return {
				club: label(team), teamId: team.id, wins: w, losses: l, ties: t, record: rec(w, l, t),
	...
	return {
		...
		worstSeasons: seasonsBy((r) => r.worstSeasons,
			(a, b) => a.winPct - b.winPct || a.losses - b.losses || a.season - b.season),
