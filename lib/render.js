...
export function leagueRecordsPage({
	...
	const gameList = (list) => list.map((g) => item(
		`${who(g)} <b>${g.pf}–${g.pa}</b> ${opponentOf(g)} <span class="dim">${escapeHtml(String(g.season))}</span>`));
...
${h === 'Championships' ? titleNote : note}</section>`).join(LF)}</div>`);
	...
	const groups = [{ label, league }, ...more];
	const body = [
		...
		...groups.map((g) => blockFor(g.league, groups.length > 1 ? g.label : null)),
	];
	if (switcher) body.push(switcher);
...
<span class="when dim">${escapeHtml(when)}</span>
	...
	const blockFor = (schedule, periodNoun, label) => {
		const out = [];
		if (label) out.push(`<h2 class="league-heading">${escapeHtml(label)}</h2>`);
...
${link(schedule.seasons.at(-1), '»', at >= 0 && at < schedule.seasons.length - 1)}
	...
	const groups = [{ label, schedule, periodNoun }, ...more];
	const body = [
