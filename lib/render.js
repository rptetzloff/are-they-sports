...
<span class="when dim">${escapeHtml(when)}</span>
	...
	const blockFor = (schedule, periodNoun, label, resolve) => {
		...
		out.push(schedule.periods.map((p) => `<section class="record-card league-wide">
...
${link(schedule.seasons.at(-1), '»', at >= 0 && at < schedule.seasons.length - 1)}
	...
	const groups = [{ label, schedule, periodNoun, resolve }, ...more];
	const body = [
		`<h1>${escapeHtml(heading)} ${escapeHtml(String(schedule.season ?? ''))}</h1>`,
		leagueNav('schedule'),
		tabs,
		nav,
		...groups.map((g) => blockFor(g.schedule, g.periodNoun, groups.length > 1 ? g.label : null, g.resolve ?? resolve)),
	];
	body.push(nav);
	if (switcher) body.push(switcher);
