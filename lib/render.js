...
export function leagueRecordsPage({
	league, heading, colors, clubs = [], mixedSports = false,
	...
	const opponentOf = (g) => {
		...
		const url = g.opponentId ? base.get(g.opponentId) : null;
...
<span class="title-chips">${seasons.map((y) => `<span class="title-chip ${RANK(name) === 0 ? 'sb' : 'title'}">${escapeHtml(String(y))}</span>`).join('')}</span>
	...
	body.push(`<div class="records">${cards.map(([h, items, note = '']) => `<section class="record-card">
<h2>${escapeHtml(h)}</h2>
${items.length ? `<ol>${items.join('')}</ol>` : '<p class="dim">None on record.</p>'}
