...
export function siteNav(base, team, { league = false } = {}) {
	...
	// The leaders page — `/coaches` or `/managers` by sport — is NOT here, and
	// was, linked from every club page and answering 404. It needs a curated
	// coaches/managers table that nobody publishes, so the link went in ahead of
	// the page and nothing noticed: `reachable.test.js` asks whether every route
	// is linked, and never whether every link is a route. `leaderPlural` stays on
	// the sport for when the table exists.
	void leaders;
...
function periodNav(schedule, periodNoun, base = '') {
	...
	const href = (p) => `${escapeHtml(base)}/schedule/${season}/${escapeHtml(p.key)}`;
	const link = (p, text, on) => (on && p
		? `<a href="${href(p)}">${escapeHtml(text)}</a>`
		: `<span class="dim">${escapeHtml(text)}</span>`);
	return `<nav class="season-nav period-nav">
${link(periods[0], '«', index > 0)}
${link(periods[index - 1], '‹', index > 0)}
<b>${periodTitle(periods[index] ?? periods[0], periodNoun)}</b>
${link(periods[index + 1], '›', index >= 0 && index < periods.length - 1)}
${link(periods.at(-1), '»', index >= 0 && index < periods.length - 1)}
</nav>
<p class="dim">${escapeHtml(`${periodNoun === 'Week' ? 'Week' : 'Day'} ${index + 1} of ${periods.length}`)} · <a class="season-link" href="${escapeHtml(base)}/schedule/${season}?all=1">show the whole season</a></p>`;
}
...
<td>${escapeHtml(pct(c.pct, 3))}</td><td class="dim">${escapeHtml(gamesBack(c.gb))}</td>
	...
	const link = (yr, text, on) => (on && yr != null
		? `<a href="${escapeHtml(base)}/standings/${yr}">${escapeHtml(text)}</a>`
		: `<span class="dim">${escapeHtml(text)}</span>`);
	const nav = seasons.length ? `<nav class="season-nav">
${link(seasons[0], '«', at > 0)}
${link(seasons[at - 1], '‹', at > 0)}
<b>${escapeHtml(shown != null ? String(shown) : seasonsShown.join(' / ') || '—')}</b>
${link(seasons[at + 1], '›', at >= 0 && at < seasons.length - 1)}
${link(seasons.at(-1), '»', at >= 0 && at < seasons.length - 1)}
</nav>` : '';
...
<tbody>${rows}</tbody></table></section>` : '',
		// The gap, said rather than left as an absence. The football site draws
		// coach-era bands across this chart; that needs a coaches table this repo
		// has no source for, and it is the same gap as the missing leaders page.
		`<p class="dim">Coach eras are not shown: this deployment has no ${escapeHtml(team.nouns.leaderPlural)} data.</p>`,
	];
