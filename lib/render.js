...
export function headToHeadPage({
	...
	/** The share row, already built. Empty renders nothing. */
	share = '',
...
<tbody>${LF}${rows.map(row).join(LF)}${LF}</tbody></table></section>`
	...
	// Beside the switcher and below the content: both are things a reader does
	// WITH the page rather than things the page says.
	if (share) body.push(share);
...
export function leadersPage({
	...
	/** The share row, already built. Empty renders nothing. */
	share = '',
	/** How many stand-ins there are with nothing hidden.
	 *
	 *  Counted from `leaders` when not given, which is right until somebody
	 *  hides them -- and then the count is zero, the link vanishes, and there is
	 *  no way back to the full list. Same shape as `total` on the head-to-head
	 *  page and the same mistake made twice.
	 */
	standIns = null,
	...
	const row = (r) => `<tr>
<td class="dim">${escapeHtml(r.label ?? '')}</td>
...
<td>${basisCell(r)}</td>
	...
	//
	// Every other parameter survives, so hiding stand-ins does not throw away
	// the column somebody sorted by -- the same rule `sortHref` follows going
	// the other way.
	const hiding = params?.get?.('interim') === 'hide';
	const withoutInterim = new URLSearchParams(params ?? '');
	withoutInterim.set('interim', 'hide');
	const showingAll = new URLSearchParams(params ?? '');
	showingAll.delete('interim');
	const href = (p) => {
		const qs = p.toString();
		return escapeHtml(qs ? `${path}?${qs}` : path || '');
	};
	const standInCount = standIns ?? leaders.filter((r) => r.interim).length;
	const interimToggle = standInCount
		? `<p class="meta">${hiding
			? `<a href="${href(showingAll)}">Show ${standInCount} interim ${escapeHtml(standInCount === 1 ? noun.replace(/e?s$/, '') : noun)}</a>`
			: `<a href="${href(withoutInterim)}">Hide ${standInCount} interim ${escapeHtml(standInCount === 1 ? noun.replace(/e?s$/, '') : noun)}</a>`}</p>`
		: '';
	...
	const body = [
		...
		// in an image.
		// Show or hide the stand-ins, as a link rather than a checkbox.
		//
		// Both sites use a checkbox and remember it in localStorage. A link
		// needs no script and no storage, and the filtered view is a URL: a
		// reader who wants "the fifteen head coaches" can send that page to
		// somebody. What it costs is that the choice does not follow them to the
		// next club, which the sites' version does.
		//
		// Drawn only where there is a stand-in to hide. A control that can never
		// change the table is worse than no control -- the same call the
		// head-to-head "current franchises" checkbox makes.
		interimToggle,
...
<tbody>${leaders.map(row).join('')}</tbody></table>
	...
	// WITH the page rather than things the page says.
	//
	// LAST, after the site nav, which is where `clubPage` puts it. This page had
	// it between the two -- one page out of five putting the same row in a
	// different place, which nobody would notice and everybody would copy.
	if (share) body.push(share);
