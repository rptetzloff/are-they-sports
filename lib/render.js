...
export function creditLine(credits) {
	...
	const items = credits.map((c) => (c.note
		? `${link(c)} <span class="dim">${escapeHtml(c.note)}</span>`
		: link(c)));
	const notices = credits.filter((c) => c.notice)
		.map((c) => `<p class="notice">${escapeHtml(c.notice)}</p>`);
...
export const historyColumns = (team) => [
	...
	// The title marker. Nothing to order by that a reader would ask for.
	{ key: null, label: '' },
];
