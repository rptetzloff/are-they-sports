...
export function recordsPage({ team, colors, records, resolve, base, siteNavHtml = '', switcher = '' }) {
	...
	const seasonList = (list) => list.map((r) => item(
		`<b>${escapeHtml(String(r.season))}</b> ${escapeHtml(r.record)}`));
	const startList = (list) => list.map((r) => item(
		`<b>${r.games}</b> to start <span class="dim">${escapeHtml(String(r.season))}</span>`));
	const streakList = (list) => list.map((r) => item(
		`<b>${r.games}</b> <span class="dim">${escapeHtml(String(r.startSeason))}${r.startSeason === r.endSeason ? '' : `–${escapeHtml(String(r.endSeason))}`}</span>`));
	const gameList = (list) => list.map((g) => item(
		`<b>${g.pf}–${g.pa}</b> ${name(g.opponent, g.season)} <span class="dim">${escapeHtml(String(g.season))}</span>`));
	const titleList = (list) => list.map((a) => item(
		`<b>${escapeHtml(String(a.season))}</b> ${a.won ? 'won' : 'lost'} <span class="dim">${escapeHtml(a.title ?? team.nouns.championship)}</span>`));
	const playoffList = (list) => list.map((a) => item(
		`<b>${escapeHtml(String(a.season))}</b> ${escapeHtml(a.record)}${a.championship ? ' <span class="dim">title game</span>' : ''}`));
