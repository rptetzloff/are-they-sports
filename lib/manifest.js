...
export const REQUIRED_RULES = ['streaksSpanSeasons', 'losslessSeasonIsPlausible', 'onThisDayWindowDays', 'schedulePeriod'];
...
export function resolveTeam(team, sport) {
	...
	const resolved = {
		...
		// Replaced, never merged. This is an ORDERED selection, and spreading two
		// arrays would append rather than override -- a club wanting eleven of the
		// sport's twelve cards would silently get twenty-three.
		//
		// Not required, and null means "everything the renderer can draw", so a
		// sport that has not thought about it yet renders the whole catalogue
		// instead of an empty page.
		records: team.records ?? defaults.records ?? null,
	};
	...
	// A selection that is present and not a list of slugs is a configuration
	// error, and the failure it prevents is the page rendering zero cards: an
	// empty record book looks like a club with no history rather than a typo.
	if (resolved.records !== null
		&& (!Array.isArray(resolved.records) || !resolved.records.length
			|| resolved.records.some((r) => typeof r !== 'string' || !r))) {
		missing.push('records (not a non-empty list of slugs)');
	}
