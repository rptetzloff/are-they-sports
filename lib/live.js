 ...
 *  which one refreshes. Without it every replica would fetch the same nine URLs
 *  on the same schedule and write the same rows — which is the objection
 *  CLAUDE.md raised against a file-per-container database, arriving from the
 *  other direction.
 */
...
export async function refreshLive(client, sportId, sport, { season = null, now = new Date() } = {}) {
	...
	for (const month of cfg.months) {
		const url = cfg.url(`${year}${String(month).padStart(2, '0')}`);
		try {
			...
			// One month failing must not lose the others, and a feed being
			// briefly unreachable is not a reason to stop refreshing forever.
			failed.push(`${year}-${month}: ${e.message}`);
		}
	}
	if (!rows.length) return { ran: true, season: year, written: 0, failed };

	// One row per id: the feed's month window overlaps at the boundary, and
	// Postgres refuses an upsert whose own values repeat a key.
	const byId = new Map(rows.map((r) => [r.id, r]));
	const written = await writeGames(client, sportId, [...byId.values()]);
	return { ran: true, season: year, written, failed };
}
