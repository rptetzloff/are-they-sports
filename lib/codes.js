...
export function codeTable(sportId, rows = loadHistory(sportId)) {
	...
	const era = new Map();
	// Any code -> the whole row that declared it, so a caller can ask for
	// another provider's spelling of the same club. An id is built from ONE
	// provider's codes and read by another: a live NFL row has to be keyed the
	// way nflverse keys it, `2024_22_KC_PHI`, or the authoritative load writes a
	// second row instead of replacing it.
	...
	return {
		...
		/** Another provider's spelling of the same club, from the same row.
		 *
		 *  `codeIn('WSH', 'nflverse')` is `WAS`. Falls back to the club's own
		 *  code, which is right for every club a provider spells the same way —
		 *  the column is only filled where it differs. */
		codeIn: (code, provider) => {
			const row = rowFor.get(code);
			if (!row) return code;
			return row[`${provider}Abbrv`] || row.teamAbbrv || code;
		},
