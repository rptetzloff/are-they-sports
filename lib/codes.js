...
import { loadHistory } from './names.js';

/** The two history files name their columns differently, and the baseball one
 *  names them misleadingly: `teamName` is a CODE (SE1, MLA) and `team` is the
 *  nickname. This was written against the football columns alone, so the MLB
 *  table came out empty — every row skipped, every code resolving to itself,
 *  and no error anywhere. Identity is exactly what this file replaced, so the
 *  bug was invisible: baseball simply kept its old behaviour.
 *
 *  Sniffed from the header rather than mapped by sport id, so a third sport's
 *  file works if it follows either convention, and says so if it follows
 *  neither. */
const SHAPES = [
	{ code: 'teamAbbrv', franchise: 'franchiseAbbrv' }, // football
	{ code: 'teamName', franchise: 'franchiseName' }, // baseball
];

function shapeOf(rows, sportId) {
	const header = Object.keys(rows[0] ?? {});
	const shape = SHAPES.find((s) => header.includes(s.code) && header.includes(s.franchise));
	if (shape) return shape;
	// A reference file that cannot be read is a configuration error, not a data
	// gap: no amount of running a build fixes it, so it dies at boot naming the
	// columns it found rather than serving a silently empty table.
	throw new Error(
		`${sportId} franchise history has no code column; found ${header.join(', ') || '(no rows)'}`);
}

/** A sport's code table: aliases in, canonical franchises out. */
export function codeTable(sportId, rows = loadHistory(sportId)) {
	const cols = shapeOf(rows, sportId);
	...
	for (const r of rows) {
		...
		// which is most of them. Falling back to the code is what makes a
		// one-era club resolve at all.
		const franchise = r[cols.franchise] || code;
