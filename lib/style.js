...
/* Coach eras on the history chart.
   ...
   ground. Alternation says "a different man from the one beside him", which is
   the whole job.

   Very low alpha on purpose. The bands sit BEHIND the line and the line is the
   subject; at .09 the boundaries read and the plot does not turn into stripes. */
.era rect { fill: var(--accent); opacity: .05; }
.era.alt rect { opacity: .09; }
.era-label {
	font-size: 11px;
	fill: var(--accent);
	opacity: .65;
...
/* The interim mark. Superscript so it reads as a footnote rather than as part
   of the name, and in the accent because a dimmed asterisk beside a bold name
...
/* A season's final. Won reads as the club's own colour; lost is dimmed rather
   than red -- reaching the final is not a bad season, and --loss beside a 15-1
   record says the wrong thing. */
.final-won { color: var(--accent); font-weight: 700; }
.final-lost { opacity: .55; }
...
/* A table wider than the phone scrolls INSIDE its own panel, rather than
   pushing the whole page sideways.

   Measured, not assumed. At 390px four pages overflowed -- /history by 59px,
   /vs by 41, /coaches by 174 and /managers by 76 -- and every one of them was a
   table. The rest of the site already fit: the club page, the season page,
   /records, /schedule, /standings and /champions. A page that scrolls sideways
   also moves the centred heading off centre, so the visible symptom is a
   clipped title and the cause is three elements down.

   On the CONTAINERS and not on a wrapper element, so a table added later is
   covered without anybody remembering to wrap it. Both sites use a wrapper
   (.h2h-table-wrap) and have exactly that to remember.

   Safe against the one thing overflow clips, which is positioned descendants:
   the standings modal is position: fixed and is a SIBLING of these, not a
   child. The focus ring on .record-card-focus is the element's own box-shadow,
   and overflow does not clip an element's own shadow.
   ...
   here is a number somebody came for. */
.panel,
.record-card {
	overflow-x: auto;
}
...
/* The fallback for the copy button this has no script to provide. It has to be
   ...
   --base-deep needs a fallback and did not have one. paletteCss emits it only
   for a club that HAS a deep variant, and most do not — so for those clubs this
   background resolved to nothing and the field sat transparent over the panel.
   An unset custom property is not an error, which is why it looked fine. */
.share-url input {
	flex: 1;
	min-width: 0;
	font: inherit;
	...
	padding: .25rem .4rem;
	color: var(--text);
	background: var(--base-deep, var(--base));
	border: 1px solid var(--panel-edge);
	border-radius: 4px;
}

/* The head-to-head filter form.

   A definite width, not only a max-width, for the reason .records and .clubs
   both carry it: body is a centred flex column, so a child sized only by
   max-width is laid out shrink-to-fit, and a wrapping flex row inside it then
   collapses to one control per line at every viewport. That is the bug at the
   top of CLAUDE.md, and it reaches every new container. */
.filters {
	width: 100%;
	max-width: 900px;
	display: flex;
...
.filters .field { display: flex; flex-direction: column; gap: .2rem; }
.filters .field > span {
	font-size: .65rem;
	letter-spacing: .08em;
	text-transform: uppercase;
...
/* The checkbox reads left-to-right, so it opts out of the column the other
   fields use rather than getting a label floating above a tickbox. */
.filters .field.check { flex-direction: row; align-items: center; gap: .4rem; }
.filters .field.check > span { text-transform: none; letter-spacing: 0; font-size: .8rem; opacity: .8; }
...
/* A select on a dark ground draws its own list in the platform's colours, and
   an unstyled option is black text on white in the middle of this page. */
.filters select option { color: var(--text); background: var(--base-deep, var(--base)); }
...
/* Only when something is filtered, so it is never a link back to the page the
...
.filters .clear:hover { color: var(--accent); opacity: 1; }
...
/* Below 600px the page's own gutters are worth more as table than as margin.
   The breakpoint is where both sites put theirs. This does not eliminate the
   scroll on the widest table and is not meant to -- it buys back about 60px,
   which is enough for /history and /vs to stop scrolling at all. */
@media (max-width: 600px) {
	table.schedule td, table.schedule th,
	table.h2h td, table.h2h th { padding: .35rem .4rem; }
	.league-table th, .league-table td { padding: .3rem .4rem; }
	.record-card { padding: .8rem .6rem; }
	.panel { padding: .7rem .5rem; }

	/* Cells stop wrapping once the table can scroll, because wrapping is worse
	   than scrolling here: the Titles cell is "1961, 1962, 1965, 1966, 1967",
	   which broke onto five lines and made Lombardi's row five rows tall --
	   while the column itself was scrolled out of sight, so the reader saw an
	   unexplained gap next to two coaches and nothing to explain it. Making the
	   table wider costs nothing when it is already a scroll container. */
	.league-table th, .league-table td,
	table.schedule td, table.schedule th { white-space: nowrap; }
}
