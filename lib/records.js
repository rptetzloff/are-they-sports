...
/** ".571", the way every record book writes a winning percentage.
 *
 *  Three decimals and no leading zero, which is how both sports write it.
 *
 *  This is notation, not precision, and the first draft of this comment claimed
 *  otherwise — that one decimal of a percentage collapsed clubs onto the same
 *  number. It does not: "57.1" and ".571" carry exactly the same three
 *  significant figures, and the Bears at .5522 against the Vikings at .5499
 *  were already distinguishable as 55.2 and 55.0. What changed is that a
 *  reader of a record book expects .571 and has to translate 57.1.
 *
 *  A perfect record keeps its leading 1, because "1.000" is a number anyone
 *  reads instantly and ".000" would be the opposite one.
 */
export const pct = (p) => (p >= 1 ? p.toFixed(3) : p.toFixed(3).replace(/^0/, ''));
