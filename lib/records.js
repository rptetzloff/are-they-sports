...
export function computeRecords(rows, { top = 5, streaksSpanSeasons = true, titles = [] } = {}) {
	...
	// Which seasons ended in a title, so a history chart can mark them without
	// recomputing the postseason. Keyed by season because that is what the
	// chart plots.
	const titleSeasons = new Set(
		games.filter((g) => g.championship && g.championship.trim() && g.result === 'WIN')
			.map((g) => parseInt(g.season, 10)));
	...
	for (const t of stated) titleSeasons.add(Number(t.season));
