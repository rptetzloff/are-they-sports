...
export function computeRecords(rows, { top = 5, streaksSpanSeasons = true } = {}) {
	...
	// Settled seasons only, and the tiebreakers are the site's: win percentage,
	// then the count that makes the season more extreme, then the earlier year.
	...
	const playoffAppearances = [...bySeasonPost.entries()]
		.map(([season, list]) => {
			...
			return {
				...
				title: titles[0]?.championshipTitle ?? null,
			};
		})
	...
	return {
		...
		championshipAppearances: playoffAppearances
			...
			.map((a) => ({ season: a.season, won: a.won, record: a.record, title: a.title })),
	};
}
