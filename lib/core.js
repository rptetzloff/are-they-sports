...
export function streakBanner(completedGames, { isPastSeason = false, team } = {}) {
	...
	if (isPastSeason) {
		if (!firstLoss) {
			...
			return `Finished the regular season undefeated — ${t ? `${w}-0-${t}` : `${w}-0`}`;
		}
		if (openingStreak === 0) return 'Lost the opener — undefeated for 0 games to start the season';
		...
		return `Undefeated for ${plural(openingStreak, 'game')} (${plural(daysToLoss(), 'day')}) to start the season before first loss`;
	}
	...
	if (!firstLoss) {
		...
		return `Undefeated to start the season — ${openingStreak}-game ${tied ? 'unbeaten run' : 'win streak'}`;
	}
	if (openingStreak === 0) return `Lost the opener. Currently on a ${winStreak}-game win streak.`;
	return `The ${team.nouns.team} started the season undefeated for ${plural(openingStreak, 'game')} (${plural(daysToLoss(), 'day')}). Currently on a ${winStreak}-game win streak.`;
}
