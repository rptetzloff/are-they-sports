...
export function streakBanner(completedGames, { isPastSeason = false, team } = {}) {
	...
	// What the club is ON right now, which is not always a win streak. This
	// counted trailing WINs only, so a club whose last game was a loss read
	// "Currently on a 0-game win streak" — a sentence nobody would write, live
	// on the Brewers' page on 31 August 2026 at 85-52. It survived because every
	// fixture in the suite ends in a win: WWLWW, WWWWW, LWWW.
	const last = sorted.at(-1).result;
	let run = 0;
	for (let i = sorted.length - 1; i >= 0 && sorted[i].result === last; i--) run++;
	const current = last === 'WIN' ? `on a <strong>${run}-game</strong> win streak`
		: last === 'LOSS' ? `on a <strong>${run}-game</strong> losing streak`
		// Consecutive ties are close to unheard of and the phrasing still has to
		// exist, because a tie is neither and saying "0-game win streak" about
		// one is how this bug read in the first place.
		: run === 1 ? 'coming off a <strong>tie</strong>' : `on a <strong>${run}-game</strong> run of ties`;
	...
	if (openingStreak === 0) return `Lost the opener. Currently ${current}.`;
	return `The ${escapeHtml(team.nouns.team)} started the season undefeated for <strong>${plural(openingStreak, 'game')}</strong> (${plural(daysToLoss(), 'day')}). Currently ${current}.`;
}
