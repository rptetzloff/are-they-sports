 ...
 *
 *  Deliberately small. These are the ones the two live sites actually have,
 ...
 *
 *  `leaderPlural` is passed in rather than looked up, because the leaders page
 *  is `/coaches` for football and `/managers` for baseball and this file must
 *  not learn which sport it is routing. Omitted, the route simply does not
 *  exist — which is the right answer for a caller that has no club in hand.
 */
export function parseView(rest, { leaderPlural = null } = {}) {
	// Before the pattern matches below, because a sport whose noun collided with
	// a fixed route would otherwise silently lose its leaders page. None does
	// today; `coaches` and `managers` are neither `records` nor `history`.
	if (leaderPlural && rest === `/${leaderPlural}`) return { view: 'leaders' };
	// Social cards. One route rather than the six the two sites carry, because
	// they are the same picture with different words in it -- and six routes
	// would be six places for the words to drift from the pages they describe.
	//
	// The slug names a view this club already has: `default`, a season, or one of
	// the fixed pages. It is validated here rather than in the renderer, so an
	// invented slug is a 404 instead of a card confidently captioned "undefined".
	const card = rest.match(/^\/og\/([a-z0-9-]+)\.png$/);
	if (card) {
		const slug = card[1];
		const known = slug === 'default' || /^\d{4}$/.test(slug)
			|| slug === 'records' || slug === 'history' || slug === 'vs'
			|| (leaderPlural && slug === leaderPlural);
		return known ? { view: 'card', card: slug } : null;
	}
	...
	if (rest === '/history') return { view: 'history' };
