...
/** The franchises still playing, by canonical code.
 *
 *  Read out of the franchise history table rather than listed. The football
 *  site hardcodes thirty-one names in `h2h-core.js` and the baseball site
 *  derives it from the games instead -- a franchise is "current" there if it
 *  appears in the latest season present in the data. Neither rule ports:
 *  the hardcoded list goes stale the next time a club moves, and the derived
 *  one is only safe in a sport where everybody plays everybody. A football club
 *  meets fourteen of the other thirty-one in a season, so "last met this year"
 *  would call twenty-eight franchises defunct.
 *
 *  The rule here is the table's own: a franchise is current if its latest era
 *  is the latest era anyone has. Football writes an end season on every row and
 *  the live ones read 2026; baseball leaves the end blank on an open era. Both
 *  fall out of the same comparison, with a blank sorting last on purpose.
 *
 *  Measured: 32 of 119 football franchises, and 30 of 30 baseball ones -- which
 *  is why the baseball site has no such filter. A control that can never narrow
 *  anything is worse than no control, so the page asks this before drawing one.
 */
export function currentFranchises(sportId, dir = REFERENCE_DIR) {
	const rows = loadHistory(sportId, dir);
	const index = sportId === 'nfl' ? nflIndex(rows) : mlbIndex(rows);
	// An open-ended era sorts above every dated one. Comparing as strings works
	// for both sports because football's seasons are four digits and baseball's
	// dates are ISO -- both already sort lexically the way they sort in time.
	const OPEN = '\uffff';
	const end = (span) => (span.to === '' || span.to === null || span.to === undefined
		? OPEN : String(span.to));
	const latest = new Map();
	for (const [, spans] of byFranchise(index)) {
		for (const span of spans) {
			const seen = latest.get(span.franchise);
			if (seen === undefined || end(span) > seen) latest.set(span.franchise, end(span));
		}
	}
	const newest = [...latest.values()].sort().at(-1);
	return new Set([...latest].filter(([, v]) => v === newest).map(([k]) => k));
}
